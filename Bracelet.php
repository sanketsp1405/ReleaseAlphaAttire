...
<body>
    ...
    <div class="container">
      ...
      $userId = $_SESSION['user_id']; // Retrieve user ID from session
        ...
        // SQL query to fetch product data where sub_category_id = 15
        ...
        if ($result->num_rows > 0) {
           ...
           <div class="card" data-price="<?php echo $row['price']; ?>" data-brand="<?php echo $row['brand']; ?>" data-size="<?php echo $row['size']; ?>">
                    ...
                    <button class="add-to-wishlist" onclick="addToWishlist('<?php echo $row['id']; ?>','<?php echo htmlspecialchars($row['name']); ?>', '<?php echo $row['price']; ?>', '<?php echo htmlspecialchars($row['image1']); ?>'); event.stopPropagation();">Add to Wishlist</button>
                </div>
                <?php
            }
        } else {
    ...
    <script>
        function filterProducts() {
            ...
            cards.forEach(card => {
                const cardPrice = parseFloat(card.getAttribute('data-price').replace(/,/g, '')); // Remove commas from price
